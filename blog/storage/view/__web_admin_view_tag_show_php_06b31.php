                        ...
                        <form class="am-form" action="" method="post">
                            <table class="am-table am-table-striped am-table-hover table-main">
                                ...
                                <?php foreach ((array)$data as $key=>$value){?>
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="check" value="<?php echo $value['tid']?>">
                                        </td>
                                        <td><?php echo $key+1?></td>
                                        <td><a href="#"><?php echo $value['tname']?></a></td>
                                        <td>
                                            <div class="am-btn-toolbar">
                                                <div class="am-btn-group am-btn-group-xs">
                                                    <button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="edit(<?php echo $value['tid']?>)"><span class="am-icon-pencil-square-o"></span> 编辑</button>
                                                    <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="del(<?php echo $value['tid']?>)"><span class="am-icon-trash-o"></span> 删除</button>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <div class="am-cf">
                                共 <b class="count" style="color: #ff0000;"><?php echo count($data)?> </b>条记录
                                <div class="am-fr">
                                    <?php echo $page?>
                                </div>
                            </div>
...
<script>
    ...
    function edit(tid) {
        window.location.href = "<?php echo u(edit)?>" + '&tid=' + tid;
    }
    $(function(){
        $('.select').change(function(){
            var tid = $(this).val();
            $.post('<?php echo u(showCategory)?>',{tid:tid},function (res) {
                if(res){
                    $('.tbody').html(res);
        ...
        require(['util'], function (util) {
            var obj = util.modal({
                ...
                events:{
                    confirm:function(){
                        //哪个元素上有.confirm类，被点击就执行这个回调
                        location.href = "<?php echo u(delAll)?>" + "&tidAll=" + str;
                    }
                }
            });
            //显示模态框
