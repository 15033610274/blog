                ...
                <div class="am-g">
                    <div class="am-u-sm-12">
                        <form class="am-form" action="" method="post">
                            <table class="am-table am-table-striped am-table-hover table-main">
                                ...
                                <?php foreach ((array)$oldData as $key=>$value){?>
                                    <tr>
                                        ...
                                        <td class="am-hide-sm-only"><?php echo $value['sendtime']?></td>
                                        <td class="am-hide-sm-only"><?php echo $value['author']?></td>
                                        <td class="am-hide-sm-only"><?php echo Db::table('user')->where('uid',$value['user_uid'])->pluck('username')?></td>
                                        <td class="am-hide-sm-only"><?php echo Db::table('category')->where('cid',$value['category_cid'])->pluck('cname')?></td>
                                        <td>
                                            <div class="am-btn-toolbar">
                                                <div class="am-btn-group am-btn-group-xs">
                                                    <button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="revocation(<?php echo $value['aid']?>)"><span class="am-icon-pencil-square-o"></span> 撤销</button>
                                                    <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" onclick="removeDel(<?php echo $value['aid']?>)"><span class="am-icon-trash-o"></span> 删除</button>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <hr>
                            <p>注：.....</p>
                        <input type='hidden' name='__TOKEN__' value='17b2bbe3c4a2b21fc5d71ee5718e943b'/></form>
                    </div>
...
<script>
            ...
            var obj = util.modal({
                ...
                events:{
                    confirm:function(){
                        ...
                        location.href = "<?php echo u(removeDel)?>" + "&aid=" + aid;
                    }
                }
            });
            //显示模态框
        ...
        require(['util'], function (util) {
            var obj = util.modal({
                ...
                events:{
                    confirm:function(){
                        //哪个元素上有.confirm类，被点击就执行这个回调
                        location.href = "<?php echo u(revocation)?>" + "&aid=" + aid;
                    }
                }
            });
            //显示模态框
    ...
    function search(){
        ...
        $.post('<?php echo u(searchDel)?>',{'searchText':searchText},function (res) {
            if(res != ''){
                $('.tbody').html(res);
                var length = $('.tbody input[name="check"]').length;
                $('.count').html(length);
        ...
        require(['util'], function (util) {
            var obj = util.modal({
                ...
                events:{
                    confirm:function(){
                        //哪个元素上有.confirm类，被点击就执行这个回调
                        location.href = "<?php echo u(delAll)?>" + "&aidAll=" + str;
                    }
                }
            });
            //显示模态框
